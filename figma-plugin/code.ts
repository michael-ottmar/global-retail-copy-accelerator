...
let processedFrames: FrameData[] = [];
let imageExports: Map<string, string> = new Map();
...
async function scanFrames() {
  ...
  function findFrames(node: SceneNode) {
...
function extractFrameData(node: FrameNode | ComponentNode): FrameData {
  ...
  if (node.cornerRadius && node.cornerRadius !== figma.mixed) {
    data.cornerRadius = node.cornerRadius as number;
  }
...
function extractNodeData(node: SceneNode): any {
  ...
  if (node.type === 'TEXT') {
    ...
    const shapeNode = node as RectangleNode | EllipseNode;
...
async function exportSelected(settings: ExportSettings) {
...
async function exportAll(settings: ExportSettings) {
  ...
  if (settings.includeImages) {
    for (const frameData of processedFrames) {
      const node = figma.getNodeById(frameData.id) as FrameNode;
      if (node) {
        await exportImages(node, settings);
      }
    }
...
async function exportImages(node: FrameNode | ComponentNode, settings: ExportSettings) {
  try {
    ...
    async function findAndExportImages(searchNode: SceneNode) {
      if ('fills' in searchNode && searchNode.fills !== figma.mixed) {
        const fills = searchNode.fills as Paint[];
...
async function sendToApp(url: string, data: any) {
